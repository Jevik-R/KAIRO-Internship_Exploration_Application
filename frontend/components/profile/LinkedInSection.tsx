...
export default function LinkedInSection({ hasLinkedIn, applicant, isOwner }: LinkedInSectionProps) {
  const [linkedInExists, setLinkedInExists] = useState(hasLinkedIn)

  const handleDelete = () => {
    setLinkedInExists(false)
  }
    ...
    <section className="bg-card border border-border rounded-xl shadow-sm p-6 sm:p-8">
      ...
      {linkedInExists ? (
        <div className="space-y-4">
          ...
          {isOwner && <LinkedInDeleteButton userId={applicant.userId} onDelete={handleDelete} />}
        </div>
      ) : (
